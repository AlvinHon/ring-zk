    ...
    T: Zero,
{
    ...
    pub(crate) fn from_vec(polynomials: Vec<Polynomial<T, N>>) -> Self {
        Mat {
    ...
    ///
    /// ## Panics
    /// Panics if the matrix dimension is not (m x 1).
    pub(crate) fn one_d_mat_to_vec(self) -> Vec<Polynomial<T, N>> {
        self.polynomials
            .into_iter()
            .map(|p| {
                assert_eq!(p.len(), 1, "Matrix dimension is not (m x 1)");
    ...
    pub(crate) fn new_with<F>(m: usize, n: usize, mut f: F) -> Self
    where
    ...
    ///
    /// ## Panics
    /// Panics if the dimensions of the two matrices are not equal.
    #[allow(clippy::needless_range_loop)]
    ...
    ///
    /// ## Panics
    /// Panics if the number of columns of the two matrices are not equal.
    ...
    ///
    /// ## Panics
    /// Panics if r is greater than the number of rows of the matrix.
    ...
    ///
    /// ## Panics
    /// Panics if the number of rows of the two matrices are not equal.
